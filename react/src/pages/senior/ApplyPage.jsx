...
const makeSlots = (duration=1)=>{
  const open=9, close=18, last=close-duration;
  const arr=[]; for(let h=open; h<=last; h++) arr.push({key:`${h}-${h+duration}`, label:`${pad(h)}:00 ~ ${pad(h+duration)}:00`});
  return arr;
};

const getApps = () => { try { return JSON.parse(localStorage.getItem('applications')||'[]'); } catch { return []; } };
const setApps = (next) => { try { localStorage.setItem('applications', JSON.stringify(next)); } catch {} };
...
export default function ApplyPage(){
  ...
  const selectedPet = useMemo(() => {
    ...
    try {
      const raw = localStorage.getItem('selectedPet');
      if (raw) {
    ...
    if (id) {
      const hit = FALLBACK_PETS.find(p => p.id === id);
      if (hit) return hit;
    }
  ...
  const [age, setAge] = useState('');           // 시니어 나이
  ...
  const canSubmit =
    ...
    days.length>0 && slot && agreeTerms && agreeBodycam;
  ...
  const handleSubmit = (e)=>{
    ...
    const app = {
      ...
      mode, days, duration, slot, note,
      status: 'PENDING',          // 매니저 → FORWARDED(보호소 검토중) → 보호소 → APPROVED
      createdAt: Date.now()
    };

    setApps([app, ...getApps()]);

    // 메인으로 가되 토스트용 payload 전달
    nav('/senior', {
      replace: true,
      state: {
        toast: {
    ...
    <div className="apply-page">
      ...
      <section className="profile">
        <div className="profile__media">
          <img src={pet.photoUrl} alt={pet.name} onError={(e)=>{ e.currentTarget.src = dog1; }} />
        </div>

        <div className="profile__body">
          <div className="profile__head">
            <h2 className="profile__title">({pet.name})</h2>
            <div className="profile__shelter">{pet.careName}</div>
          </div>
          ...
          {!!pet.temperament.length && (
            <>
              <div className="kv__label">성격/특징</div>
              <div className="chips">
                {pet.temperament.map((t,i)=><span key={i} className="chip">{t}</span>)}
              </div>
            </>
          )}
      ...
      <form className="apply__form" onSubmit={handleSubmit}>
        <fieldset className="fs">
          ...
          <label><span>이름</span>
            <input className="big" value={name} onChange={e=>setName(e.target.value)} placeholder="예) 김OO" />
          </label>
          <label><span>나이</span>
            <input className="big" value={age} onChange={e=>setAge(e.target.value)} placeholder="예) 73" />
          </label>
          <label><span>연락처(휴대폰)</span>
            <input className="big" value={phone} onChange={e=>setPhone(e.target.value)} placeholder="예) 010-0000-0000" />
          </label>
          <label><span>상세 주소</span>
            <input className="big" value={address} onChange={e=>setAddress(e.target.value)} placeholder="예) 부산 수영구 광안동 123-4, 101동 1001호" />
          </label>
          <label><span>비상 연락망(선택)</span>
            <input className="big" value={altPhone} onChange={e=>setAltPhone(e.target.value)} placeholder="가족/이웃 연락처" />
          </label>
        </fieldset>
        ...
        <fieldset className="fs">
          ...
          <div className="radio-row">
            <label className={`radio-big ${mode==='산책' ? 'on': ''}`}>
              <input type="radio" name="mode" checked={mode==='산책'} onChange={()=>setMode('산책')} />
              산책 체험(외부)
            </label>
            <label className={`radio-big ${mode==='실내' ? 'on': ''}`}>
              <input type="radio" name="mode" checked={mode==='실내'} onChange={()=>setMode('실내')} />
              실내 교류(가정/실내)
            </label>
          </div>
        </fieldset>
        ...
        <fieldset className="fs">
          <legend>희망 요일 · 시간</legend>
          <div className="days">
            {DAYS.map(d => (
              <label key={d} className={`chip-lg ${days.includes(d) ? 'chip--on' : ''}`}>
          ...
          <div className="duration">
            <span className="duration__label">희망 시간대</span>
            <div className="duration__chips">
              {[1,2,3].map(h => (
                <button key={h} type="button" onClick={()=>{ setDuration(h); setSlot(null); }}
                        className={`chip-lg ${duration===h ? 'chip--on': ''}`}>
                  {h}시간
                </button>
              ))}
            </div>
          ...
          <div className="slot-grid">
            {slots.map(s => (
              <label key={s.key} className={`slot ${slot===s.label ? 'slot--on' : ''}`}>
                <input type="radio" name="slot" checked={slot===s.label} onChange={()=>setSlot(s.label)} />
                {s.label}
              </label>
            ))}
          </div>
        ...
        <fieldset className="fs">
          ...
          <textarea
            ...
            placeholder="예) 무릎이 불편해 엘리베이터가 있는 곳에서 만났으면 합니다."
          />
        </fieldset>
        ...
        <fieldset className="fs">
          ...
          <div className="termsbox" ref={termsRef} tabIndex={0}>
            ...
            <ul>
              <li>매니저 및 보호소의 안내를 따릅니다.</li>
              <li>동물/본인 안전을 최우선으로 합니다.</li>
              <li>위험상황은 즉시 보호소/매니저에게 알립니다.</li>
              <li>동물 컨디션에 따라 일정이 변경될 수 있습니다.</li>
            </ul>
            <p><strong>개인정보 및 촬영</strong></p>
            <ul>
              <li>연락처/주소는 매칭·연락 용도로만 사용됩니다.</li>
              <li>안전 기록을 위해 바디캠을 사용할 수 있습니다.</li>
              <li>영상은 분쟁 예방 목적으로만 보관되며 외부 공개하지 않습니다.</li>
            </ul>
          </div>

          <div className="agree-rows">
            <label className="tgl">
              <input type="checkbox" checked={agreeTerms} disabled={!hasReadTerms}
                     onChange={(e)=>setAgreeTerms(e.target.checked)} />
              <span className={`tgl__track ${!hasReadTerms ? 'is-disabled':''}`} />
              <span className="tgl__label">이용 약관에 동의합니다.</span>
            </label>
            <label className="tgl">
              <input type="checkbox" checked={agreeBodycam}
                     onChange={(e)=>setAgreeBodycam(e.target.checked)} />
              <span className="tgl__track" />
              <span className="tgl__label">안전을 위한 바디캠 촬영에 동의합니다.</span>
            </label>
          </div>
        </fieldset>
